...
export default function Home() {
  ...
  const initialColorId = initialParams.get("color");
  const requestedBrand = initialParams.get("brand");
  const initialScope: CatalogScope = requestedBrand === "SHINERAY" || requestedBrand === "SBM" ? requestedBrand : "all";
  const initialCatalog = initialScope === "all" ? motos : motos.filter((moto) => moto.brand === initialScope);
  const initialModelIndex = initialCatalog.findIndex((moto) => moto.id === initialModelId);
  ...
  const visibleMotos = useMemo(() => scope === "all" ? motos : motos.filter((moto) => moto.brand === scope), [scope]);
  ...
  useEffect(() => {
    if (activeIndex >= visibleMotos.length) setActiveIndex(0);
  }, [activeIndex, visibleMotos.length]);

  useEffect(() => {
    const url = new URL(window.location.href);
    const nextMode = listMode ? "list" : opened ? "book" : "";
    const nextBrand = scope === "all" ? "" : scope;
    if (nextMode === lastUrlMode.current && url.searchParams.get("brand") === nextBrand) return;
  ...
  }, [listMode, opened, scope]);

  useEffect(() => {
    const onPopState = () => {
      const mode = new URLSearchParams(window.location.search).get("mode");
      const normalizedMode = mode === "book" || mode === "list" ? mode : "";
      const nextBrand = new URLSearchParams(window.location.search).get("brand");
      const normalizedScope: CatalogScope = nextBrand === "SHINERAY" || nextBrand === "SBM" ? nextBrand : "all";
  ...
  useEffect(() => {
    document.title = listMode
      ...
      : opened
        ? `${visibleMotos[activeIndex]?.name ?? "Catálogo"} — Neto Motos | Edição 2026`
        : "Neto Motos — Shineray | Catálogo 2026";
  }, [activeIndex, listMode, opened, visibleMotos]);
  ...
  function openCatalog(index = 0, requestedScope: CatalogScope = scope) {
    const catalog = requestedScope === "all" ? motos : motos.filter((moto) => moto.brand === requestedScope);
    const safeIndex = Math.max(0, Math.min(index, catalog.length - 1));
    setScope(requestedScope);
    trackEvent("catalog_page_view", { page: safeIndex + 1, section: requestedScope.toLowerCase(), source: safeIndex === 0 ? "cover" : "index", brand: requestedScope });
  ...
  function openQuote(source: string) {
    trackEvent("cta_click", { cta_name: "quote_open", model: "catalogo", location: source });
    setQuoteOpen(true);
  }

  function openBrand(brand: "SHINERAY" | "SBM") {
    const index = motos.filter((moto) => moto.brand === brand).findIndex((moto) => moto.brand === brand);
    if (index < 0) return;
    trackEvent("brand_select", { brand, location: "cover" });
    openCatalog(index, brand);
  }
  ...
  return (
    <div className={`site-shell ${opened ? "site-shell--opened" : ""}`}>
      ...
      {listMode ? (
        <ListMode open={listMode} onClose={() => setListMode(false)} onSelect={(index) => { setListMode(false); openCatalog(index, "all"); }} onOpenQuote={() => openQuote("list")} />
      ) : !opened ? (
        <Cover onOpen={() => openCatalog(coverIndex, "all")} onOpenBrand={openBrand} onOpenAbout={() => { setAboutOpen(true); setIndexOpen(false); }} onOpenIndex={() => { setIndexOpen(true); setAboutOpen(false); }} onOpenQuote={() => openQuote("cover")} onToggleMenu={() => setMobileMenuOpen(!mobileMenuOpen)} mobileMenuOpen={mobileMenuOpen} />
      ) : (
        <BookFrame motos={visibleMotos} activeIndex={activeIndex} initialColorId={initialColorId} onIndexChange={setActiveIndex} onOpenIndex={() => setIndexOpen(true)} onOpenAbout={() => setAboutOpen(true)} onBackToCover={() => { setScope("all"); setActiveIndex(0); setOpened(false); window.scrollTo({ top: 0, behavior: "smooth" }); }} soundEnabled={soundEnabled} onToggleSound={() => setSoundEnabled((enabled) => {
          const next = !enabled;
      ...
      {!listMode && <IndexDrawer motos={visibleMotos} scope={scope} open={indexOpen} activeIndex={activeIndex} onClose={() => setIndexOpen(false)} onSelect={(index) => { setIndexOpen(false); openCatalog(index, scope); }} onListMode={() => { setIndexOpen(false); setListMode(true); }} />}
...
function Cover({ onOpen, onOpenBrand, onOpenAbout, onOpenIndex, onOpenQuote, onToggleMenu, mobileMenuOpen }: { onOpen: () => void; onOpenBrand: (brand: "SHINERAY" | "SBM") => void; onOpenAbout: () => void; onOpenIndex: () => void; onOpenQuote: () => void; onToggleMenu: () => void; mobileMenuOpen: boolean }) {
    ...
    <main id="catalog-content" className="cover-stage">
      ...
      <div className="cover-content">
        <div className="cover-copy">
          ...
          <div className="cover-brand-picker" aria-label="Escolha uma linha de motocicletas">
            <span className="cover-brand-picker__label">EXPLORE POR LINHA</span>
            <div className="cover-brand-picker__actions">
              <button type="button" className="cover-brand-choice" onClick={() => onOpenBrand("SHINERAY")} aria-label="Abrir linha Shineray"><img className="cover-brand-choice__logo cover-brand-choice__logo--shineray" src={shinerayLogo} alt="Logo Shineray" /><span>Shineray</span><ArrowRight size={13} /></button>
              <button type="button" className="cover-brand-choice" onClick={() => onOpenBrand("SBM")} aria-label="Abrir linha SBM"><img className="cover-brand-choice__logo cover-brand-choice__logo--sbm" src={sbmLogo} alt="Logo SBM" /><span>SBM</span><ArrowRight size={13} /></button>
            </div>
          </div>
        </div>
      </div>
      <footer className="cover-footer"><span className="copyright-notice">© 2026 NETO MOTOS · TODOS OS DIREITOS RESERVADOS</span><span>ATENDIMENTO DIRETO — (11) 97847-3480</span><a className="creator-badge" href={creatorPortfolio} target="_blank" rel="noreferrer" aria-label="Conheça o trabalho de Gustavo Borges"><img src={creatorPortrait} alt="Gustavo Borges" /><span><small>CRIADO POR</small><b>GUSTAVO BORGES</b></span></a><span>SCROLL <ArrowDown size={12} /></span></footer>
    </main>
  );
}

function IndexDrawer({ motos: indexMotos, scope, open, activeIndex, onClose, onSelect, onListMode }: { motos: typeof motos; scope: CatalogScope; open: boolean; activeIndex: number; onClose: () => void; onSelect: (index: number) => void; onListMode: () => void }) {
  ...
  return (
    <div className={`overlay-panel ${open ? "overlay-panel--open" : ""}`} aria-hidden={!open} inert={!open}>
      ...
      <aside className="index-panel" role="dialog" aria-modal="true" aria-labelledby="index-title">
        <div className="index-panel__top"><span className="page-kicker">ÍNDICE / {scope === "all" ? "CATÁLOGO COMPLETO" : scope} / 2026</span><button ref={closeRef} type="button" className="icon-button" onClick={onClose} aria-label="Fechar índice"><X size={18} /></button></div>
        ...
        <div className="index-list">
          {indexMotos.map((moto, index) => <button key={moto.id} type="button" className={`index-item ${activeIndex === index ? "index-item--active" : ""}`} onClick={() => onSelect(index)} aria-current={activeIndex === index ? "page" : undefined}><span className="index-item__num">{formatChapter(index, indexMotos.length).split(" /")[0]}</span><AssetImage className="index-item__image" src={moto.images[0].src} alt="" fallbackLabel={moto.name} loading="lazy" /><span className="index-item__copy"><b>{moto.name}</b><small>{moto.brand} / {moto.category}</small></span><ChevronRight size={16} /></button>)}
        </div>
...
function ListMode({ open, onClose, onSelect, onOpenQuote }: { open: boolean; onClose: () => void; onSelect: (index: number) => void; onOpenQuote: () => void }) {
  ...
  const groupedEntries = useMemo(() => (["SHINERAY", "SBM"] as const).map((brandName) => ({
    brand: brandName,
    entries: entries.filter(({ moto }) => moto.brand === brandName),
...
function QuoteDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    trackEvent("cta_click", { cta_name: "quote_submit", model: form.model || "nao_informado", location: "quote_form" });
    trackEvent("quote_submit", { model: form.model || "nao_informado", location: "quote_form" });
    setSubmitted(true);
  }
